...
class FormatField:
    ...
    def __call__(self, form, field):
        ...
        if self.id_safe and not re.match(r"^[A-Za-z0-9.:_-]+$", test_format):
            raise ValidationError(
                _(
                    "This field may only contain alphanumeric chars, dots, colons, "
...
class PluginSettingsForm(PaymentPluginSettingsFormBase):
    ...
    markup = IntegerField(
        ...
        description=_(
            "increase the final amount by specified percentage amount to factor in Razorpay markup. 1% = 1000 "
        ),
    )
